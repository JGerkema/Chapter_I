...
# For 2204 species names, a valid match is found. 
...
# initial match. The new names contain a mix of accepted, unchecked and synonym names.
...
check <- accepted_species %>%
 ...
 # select(-c(acceptedNameUsageID)) %>%
  ...
  group_by(AccSpeciesName) %>%
  filter(n() >1)
length(unique(check$AccSpeciesName))

check <- try_filtered %>%
  filter(AccSpeciesName %in% c(multiple_synonyms$scientificName)) %>%
...
check2 <- try_filtered %>%
  filter(AccSpeciesName %in% c("Jupunba trapezifolia")) %>%
  select(AccSpeciesName, SpeciesName) %>%
  distinct()
...
check <- try_filtered %>%
  filter(AccSpeciesName %in% c(matched_synonyms$new_name)) %>%
  select(AccSpeciesName, SpeciesName) %>%
  distinct()
