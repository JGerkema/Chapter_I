...
plot_cats4_function <- function(orig_input, try_input, plot_title,
                                x_lab, y_lab, difference, figure_table){
  cats <- temp_raw_plot %>%
    filter(traits == orig_input) %>%
  ...
  signif <- temp_raw_plot %>%
    filter(traits %in% c(orig_input)) %>%
    filter(pval.meta < 0.05) 
  
  cats_both <- temp_raw_plot %>%
    filter(traits == try_input) %>%
    filter(Model_type %notin% c("trait_minus_bias", "raw_meta_trait_uncor")) %>%
    select(-c(FEve, FRic, qual.FRic, RaoQ, traits, 
              obs.stat_u)) %>%
    select(fit, pval.meta, FDiv, study_group, plot_study, Model_type) %>%
  ...
  ggplot(cats_both_signif, aes(x = fit, y = fit_try)) +
    geom_point(alpha = 0.3, aes(colour = study_group)) +
    theme_classic() +
    geom_smooth(method = "lm", se = T) + 
    labs(title = "CATS at species level with original and TRY trait values - only significant plots",  
  ...
  for(i in 1:length(model_types) ){
    ...
    model <- lmerTest::lmer(fit_try ~ fit + (1 | study_group), data = cats_one_model_type)
...
plot <-  ggplot(cats_both_models, aes(x = fit, y = fit_try)) +
    ...
    theme(legend.position = "bottom") +
    geom_ribbon(aes( ymin = (F0 - 1.96 * SE), ymax = (F0 + 1.96 * SE)), 
                alpha = 0.2, linetype= 1, size=0.5, data = cats_both_models, fill = "navy",
                colour = "navy") +
...
cats_both_dif <- cats_both %>%
  filter(Model_type %notin% c("Estimate of model bias (u)", "Metacommunity (m)",
                              "Metacommunity and traits (m,t)"))

plot_count <- cats_both_dif %>% 
  group_by(study_group) %>% 
  tally() 

stat.test <- cats_both_dif %>%
  full_join(plot_count) %>%
  mutate(study_group_n = paste0(study_group, " (", n, ")")) %>%
  group_by(study_group_n) %>%
  t_test(dif_fit ~ 1,  p.adjust.method = "bonferroni",
    paired = FALSE,
...
plot_df <- ggplot(cats_both_dif, aes(x = study_group_n, y = dif_fit)) +
  geom_boxplot(aes(fill = study_group_n)) +
  labs(title = NULL,
       x = x_lab,
       y = y_lab) +
  theme_minimal() +
