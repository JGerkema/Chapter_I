...
length(unique(TRYdata1$AccSpeciesName)) # n = 94230
length(unique(TRYdata1$AccSpeciesID)) # n = 

check <- try_filtered %>% 
  select(AccSpeciesName, AccSpeciesID) %>%
  distinct() %>%
  ...
  filter(AccSpeciesName %in% c(removed_because_nonsensical_final$AccSpeciesName)) 

check2 <- removed_because_nonsensical_final %>%
  filter(AccSpeciesName %notin% check$AccSpeciesName)
length(unique(removed_because_nonsensical_final$AccSpeciesName))

check3 <- TRYdata1 %>%
   filter(AccSpeciesID %in% check$AccSpeciesID)
...
load("~/0. PhD/0. Chapter_I/data/processed/try_clean23_1.RData")
...
# match with the WFO backbone. n = 64628.
...
unique(try_wf_clean_first$taxonRank)
...
length(unique(not_included_in_wf$AccSpeciesName)) #3104
...
length(unique(second_check_match$scientificName))
...
length(unique(try_wf_clean_second$scientificName)) #n = 754
...
save(try_filtered, try_clean2, removed_because_nonsensical, matched_with_wf,
 file = "data/processed/try_clean23_1.RData")
...
length(unique(try_wf_clean_complete$scientificName)) #65481
...
length(unique(accepted_species_single_first$wfo_match)) #58299
...
length(unique(accepted_species_single$wfo_match)) #59887
...
length(unique(accepted_species$new_try_name)) #60786
...
synonyms <- try_wf_clean_complete %>% # was eerst try_wf_clean2
  filter(taxonomicStatus == "Synonym") %>% # n = 8414
  filter(scientificName %notin% c(accepted_species$new_try_name)) %>%  # n = 4693
...
length(unique(synonyms$scientificName)) # n = 4695
...
length(unique(single_synonyms$scientificName)) # n = 4496
...
length(unique(all_synonyms$scientificName)) # n = 4623
...
length(unique(matched_synonyms$new_try_name)) # 4602
...
length(unique(combined$wfo_match)) # HET KLOPT!! 64018 unique new species names and codes
...
colSums(is.na(try_tax))
sort(unique(try_tax$order))
...
save(try_traits_tax_final, file = "data.public/try_final251.RData")
...
na_n <- (colSums(is.na(try_species_lvl_norm))) %>%
  data.frame() %>%
  rename(na_n = ".") %>%
  mutate(perc_na = na_n/nrow(try_species_lvl_norm))
